...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // 初始化9个子控件
        for (int i = 0; i<9; i++) {
            HWPhotoView *photoView = [[HWPhotoView alloc] init];
...
- (void)photoTap:(UITapGestureRecognizer *)recognizer
{
  ...
  //  HWLog(@"1233");
    ...
    for (int i = 0; i<count; i++) {
        ...
        HWPhoto *hwphoto = self.photos[i];
        NSString *photoUrl = [hwphoto.thumbnail_pic stringByReplacingOccurrencesOfString:@"thumbnail" withString:@"bmiddle"];
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<self.subviews.count; i++) {
        // 取出i位置对应的imageView
        HWPhotoView *photoView = self.subviews[i];
        ...
        if (i < photos.count) {
            ...
            CGFloat photoX = col * (HWPhotoW + HWPhotoMargin);
            CGFloat photoY = row * (HWPhotoH + HWPhotoMargin);
            photoView.frame = CGRectMake(photoX, photoY, HWPhotoW, HWPhotoH);
...
+ (CGSize)photosViewSizeWithPhotosCount:(int)count
{
    ...
    // 高度
    CGFloat photosH = rows * HWPhotoH + (rows - 1) * HWPhotoMargin;
    
    // 总列数
    int cols = (count >= maxColumns) ? maxColumns : count;
    // 宽度
    CGFloat photosW = cols * HWPhotoW + (cols - 1) * HWPhotoMargin;
